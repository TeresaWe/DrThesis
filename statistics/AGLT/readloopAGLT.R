...
for (j in sub.folders) {
  ...
  filenames = list.files(path=path,pattern="[A-Z]{2}[0-9]{2}[A-Z]{3}[0-9]{3}_AGLT_[0-9]{4}_[A-Za-z]{3}_[0-9]{2}_[0-9]{4}exp_local.csv")
  for (i in filenames) {
    name<-substr(j,1,10)
    name<-paste(name,"_AGLT_local", sep="")
...
for (j in sub.folders) {
  path<-paste(getwd(),"/",j,sep="")
  filenames = list.files(path=path,pattern="[A-Z]{2}[0-9]{2}[A-Z]{3}[0-9]{3}_AGLT_[0-9]{4}_[A-Za-z]{3}_[0-9]{2}_[0-9]{4}exp_global.csv")
  for (i in filenames) {
    name<-substr(j,1,10)
    name<-paste(name,"_AGLT_global", sep="")
    assign(name,read.csv(paste(path,"/",i,sep=""))) #read in the table and name as "name"
  }
}