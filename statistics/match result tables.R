...
index_online<-match(x=Cognition_final$code_online, table=results$code)
#find NA by hand
...
allresults<-cbind(Cognition_final,resultsindex) 
...
raw181108 <- read_csv("~/files/unipark/unipark/Workspaces/raw181108.csv")
ind<-match(x=allresults$code_online, table=raw181108$dupl1_Code)
online_data_institute<-raw181108[ind,]
...
##### export tables
setwd("~/files/data_tables")
write.csv(allresults,"allresults181212.csv")
write.csv(online_data_institute,"online_data_institute_181108.csv")

####
allresults$AQ_dichotomy<-1
allresults$AQ_dichotomy[allresults$AQ_Score>18]<-1
allresults$AQ_dichotomy[allresults$AQ_Score<=18]<-0
...
allresults<-mutate(allresults, APAQ=AQ_Score)
allresults$APAQ[allresults$AQ_dichotomy==1&allresults$AP.0.RP.1.AP.==1]<-1
allresults$APAQ[allresults$AQ_dichotomy==0&allresults$AP.0.RP.1.AP.==1]<-2
allresults$APAQ[allresults$AQ_dichotomy==1&allresults$AP.0.RP.1.AP.==0]<-3
allresults$APAQ[allresults$AQ_dichotomy==0&allresults$AP.0.RP.1.AP.==0]<-4
