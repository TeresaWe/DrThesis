...
file_list_raw<-freqlist_IS17THE058
dev_sounds2_IS17THE058 <- list()

for (j in seq_along(file_list_raw)) {
  #rm(dev_raw, vec, nam)
  dev_raw<-file_list_raw[[j]]
  PAT_table<-PAT_IS17THE058
  entered<-as.numeric(PAT_table[j,4])
  label<-PAT_table[j,6] #table
  target<-PAT_table$target.note # column name sometimes changes depending on type of read in...
  ...
  #plot
  count<-numeric(length(vec))
  count<-1:length(vec) # length(count)/10=seconds 
  vec<-data.frame(vec)
  data<-data.frame(vec,count)
  f<-ggplot(data, aes(x=count,y=vec))
  print(f+geom_line()+
          coord_cartesian(ylim=c(-1200,1200),xlim=c(0,170))+labs(title=paste(j))) #two octaves
  #vec<-list(vec)
  dev_sounds2_IS17THE058<-c(dev_sounds2_IS17THE058,vec)
  j=j+1
}
