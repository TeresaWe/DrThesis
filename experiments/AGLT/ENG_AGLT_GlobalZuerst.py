...
This experiment was created using PsychoPy2 Experiment Builder (v1.84.2),
    on Do 11 Mai 2017 16:26:48 CEST
...
foot_geninstr = visual.TextStim(win=win, name='foot_geninstr',
    ...
    text='Press green key to continue!',
    font='Arial',
...
textloop = visual.TextStim(win=win, name='textloop',
    #Ist hier der Text zu den Beispielmelodien JA, wie kann man den ändern? 
    text='default text',
    font='Arial',
...
head_locreal_2 = visual.TextStim(win=win, name='head_locreal_2',
    #text='Kurze Pause',
    text='Short break',
    font='Comic Sans',
    pos=(0, 0.75), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1,
    depth=0.0);
main_locreal_2 = visual.TextStim(win=win, name='main_locreal_2',
...
for thisGlobal_block in global_block:
    ...
    for thisBatch_global in batch_global:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        key_read5_2 = event.BuilderKeyResponse()
        # keep track of which components have finished
        Trials_PauseComponents = [head_locreal_2, main_locreal_2, key_read5_2]
        ...
        while continueRoutine:
            ...
            # *head_locreal_2* updates
            if t >= 0.0 and head_locreal_2.status == NOT_STARTED:
                # keep track of start time/frame for later
                head_locreal_2.tStart = t
                head_locreal_2.frameNStart = frameN  # exact frame index
                head_locreal_2.setAutoDraw(True)
            
            # *main_locreal_2* updates
            if t >= 0.0 and main_locreal_2.status == NOT_STARTED:
                # keep track of start time/frame for later
                main_locreal_2.tStart = t
                main_locreal_2.frameNStart = frameN  # exact frame index
                main_locreal_2.setAutoDraw(True)
            
            # *key_read5_2* updates
            if t >= 0.0 and key_read5_2.status == NOT_STARTED:
                # keep track of start time/frame for later
                key_read5_2.tStart = t
                key_read5_2.frameNStart = frameN  # exact frame index
                key_read5_2.status = STARTED
                # keyboard checking is just starting
                event.clearEvents(eventType='keyboard')
            if key_read5_2.status == STARTED:
                theseKeys = event.getKeys(keyList=['space'])
